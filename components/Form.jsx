import Link from "next/link"

const Form = ({type,post,setPost,submited,handleSubmit}) => {
  return (
    <section className='w-full max-w-full flex-start flex-col'>

      <h1 className='capitalize head_text text-left blue_gradient'>{type} post</h1>

      <p className="desc text-left max-w-md">
        {type} and share amazing prompts with the world, and let your imagination run wild
        with any AI-powered platform.
      </p>

      <form
       onSubmit={handleSubmit}
       className='mt-10 w-full max-w-2xl flex flex-col gap-7 glassmorphism'
       >

        <label>
          <span className='font-satochi font-semibold text-base text-gray-700'>
            Your AI prompt
          </span>

          <textarea
          value={post.promptText}
          onChange={(e)=>setPost({...post,promptText: e.target.value})}
          placeholder='Write your prompt here...'
          required
          className='form_textarea'
          >

          </textarea>
        </label>

        <label>
          <span className='font-satochi font-semibold text-base text-gray-700'>
            Tag {` `}
            <span>(#products,#webdevelopment...)</span>
          </span>

          <input
          value={post.tag}
          onChange={(e)=>setPost({...post,tag: e.target.value})}
          placeholder='#tag'
          required
          className='form_input'
          />

    
        </label>

        <div className="flex-end mx-3 mb-5 gap-4">
          <Link 
          href="/"
          className="text-gray-500 text-sm"
          >

          Cancel
          </Link>


          <button

          type="submit"
          disabled={submited}
          className="px-5 py-1.5 text-sm bg-primary-orange rounded-full text-white"
          >
            
            {submited ? `${type}...` : type}
            
          </button>
        </div>

       </form>

    </section>
  )
}

export default Form
